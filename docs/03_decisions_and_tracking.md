...
M1 is complete as a planning baseline. SQLite schema version 6, accounts/locking, hotspot search/creation with typed peers and GPS fallback, client encounter creation, phone-side daily summary, today's record list/detail, record delete, record edit, sync-status preparation and retention safety status are implemented. Schema version 3 adds local app/project/device identity for future dashboard sync; schema version 4 adds local dashboard connection state; schema version 5 adds pairing-preparation fields; schema version 6 adds retention check/cleanup timestamp fields for future use. P2.1 sync configuration alignment is implemented without changing the sync payload schema: the preparation screen now requires HTTPS `/api/v1`, exactly six pairing-code digits and a full certificate SHA-256 fingerprint stored through secure storage. P2.2 certificate checking is implemented so the APK can compare the live HTTPS dashboard certificate fingerprint with the approved value before later pairing or sync work. P2.3a offline preparation can build the future pairing request JSON shape and hold a future device credential in secure storage, and P2.3b can parse accepted pairing success/error responses offline, but neither sends pairing requests nor marks the phone paired. Hotspot tests cover simulated GPS, saved data, search, lock/draft preservation and worker isolation. GPS success/fallback persistence has been confirmed from the user's phone database. Client-entry tests cover fields, default values, validation, duplicate prevention and reset after save. Daily summary tests cover signed-in worker totals and unique people by client code. Today's records tests cover signed-in worker list/detail navigation, soft-delete and edit update. Security/recovery decisions are confirmed in 15_security_recovery_plan.md. Database encryption is implemented in code and accepted for development-stage phone testing; see 16_database_encryption_plan.md. Pre-pilot readiness is drafted in 17_pre_pilot_readiness.md. The joint APK/LAN sync contract is tracked in 19_joint_sync_contract_status.md and now requires a separate HTTPS phone device API on port 3443. Production sync remains unimplemented. Final verification and device evidence are tracked in 05_build_status.md.
...
| M2 Foundation | In progress | SQLite schema v6, app identity, dashboard pairing preparation, retention safety status, password verifiers and SQLCipher database opening implemented; phone migration and user data visibility checks passed |
...
| M6 Sync preparation | Pairing and retention preparation implemented; P2.1 configuration alignment, P2.2 certificate checking, P2.3a offline pairing request preparation and P2.3b offline response parsing implemented; joint APK/LAN contract drafted | See 12_sync_architecture_decision.md, 13_dashboard_api_contract.md, 19_joint_sync_contract_status.md and 20_phase2_apk_sync_implementation_plan.md; live P2.3-P2.8 remain unauthorized |
...
- Sync channel is Android APK to Windows dashboard over the office local network using a separate HTTPS device API on port 3443, with manual worker-triggered upload and per-operation acknowledgement. The existing browser dashboard may remain on HTTP port 3000 for synthetic integration UAT only; before real Outreach reporting is shown in the browser, the PM must migrate browser traffic to HTTPS or formally accept/document the HTTP risk.
...
Client encounter creation, daily summary, today's list/detail, record delete, record edit, sync-status placeholder, pairing-preparation storage, retention safety status, P2.1 sync configuration alignment, P2.2 certificate checking, P2.3a offline pairing request preparation, P2.3b offline response parsing and SQLCipher database-opening code are complete for this increment. Sync architecture is locked in 12_sync_architecture_decision.md, the future dashboard API handover contract is drafted in 13_dashboard_api_contract.md, a current worker guide is available in 14_worker_guide.md, security/recovery decisions are confirmed in 15_security_recovery_plan.md, database encryption status is in 16_database_encryption_plan.md, pre-pilot readiness is drafted in 17_pre_pilot_readiness.md, release-signing setup is documented in 18_release_signing_plan.md, the joint APK/LAN sync contract status is recorded in 19_joint_sync_contract_status.md, and the APK Phase 2 sync implementation plan is drafted in 20_phase2_apk_sync_implementation_plan.md. The next APK sync step is live P2.3 pairing transport and state application only after separate authorization and LAN readiness. Desktop sync remains a separate future integration.
